...
## Инфо-пробел `uc_dynamic_spring.uc.js`

Модульный UI-контейнер для панели инструментов Firefox.<br>Благодаря гибкой конфигурации скрипт полностью адаптируется: может работать как компактный блок с системными метриками и погодой, так и как классический расширяющийся пробел с заголовком активной вкладки.<br>Все элементы опциональны.
* [Ссылка на страницу](userChromeJS/uc_dynamic_spring.uc.js)
* [Ссылка на raw](https://raw.githubusercontent.com/KotDaVinci-1/FirefoxChromeScripts/main/userChromeJS/uc_dynamic_spring.uc.js)
### Возможности (Модули)
Все модули можно гибко настроить, отключить или разместить слева/справа от центрального пробела через блок `CONFIG` в начале скрипта.

*   **BASIS (Основа):** Три режима работы — от простого контейнера с виджетами до полноценного расширяющегося пробела с заголовком активной страницы по центру.
*   **Метрики ресурсов (MEMORY, CPU, GFX):** Отслеживание потребления оперативной памяти и процессорного времени активной вкладки, общей нагрузки на графическую подсистему браузера.
*   **Дата и Праздники (DATEW & POVOD):** Вывод текущей даты, в подсказке показывает праздники на сегодня и завтра (или ближайший праздник).
*   **Погода (WEATHER & WICONS):** Отображение температуры и нативных иконок погоды Firefox.
### Конфигурация
Настройка осуществляется правкой объекта `CONFIG` внутри самого скрипта. Вы можете назначить модулям значения `1` (слева), `2` (справа) или `0` (отключить).
### Настройка виджета погоды
Данные для погодного модуля берутся из внутренних систем Firefox. По идее, всё должно работать «из коробки» (если при первом запуске вы видите `--°C`, обычно достаточно просто подождать или перезапустить браузер, чтобы данные скачались).<br>Однако при первом запуске важно проверить, для какого именно города браузер получает прогноз. Узнать это можно двумя способами:
1. Открыть файл `activity-stream.weather_feed.json` в папке вашего профиля Firefox и посмотреть текстовые данные внутри.
2. Посмотреть название населенного пункта на виджете погоды на стандартной новой вкладке браузера (`about:newtab`).

Если город определился неверно или данные вообще не подтягиваются, выполните следующие шаги:

<details>
<summary><b>Настройка и решение проблем с погодой</b></summary>

Для изменения города или инициализации модуля необходимо воспользоваться стандартной новой вкладкой браузера:
1. **Включите стандартную новую вкладку:** Если у вас установлено расширение, заменяющее пустую страницу, временно отключите его. Перейдите в настройки `about:preferences#home` (вкладка *«Домашняя страница и запуск»*) и в пункте *«Новые вкладки»* выберите **Домашняя страница Firefox (по умолчанию)**.
2. **Измените локацию:** Откройте новую вкладку (`about:newtab`), найдите виджет погоды (обычно в левом верхнем углу). Нажмите на `•••` на виджете ➔ *«Изменить местоположение»* и задайте нужный город вручную.
3. **Что делать, если виджета нет на странице:** 
   * Откройте `about:config` и вбейте в поиск `weather`. Убедитесь, что все параметры находятся в значении *«по умолчанию»*. 
   * Проверьте параметр `browser.newtabpage.activity-stream.system.showWeather` — он должен быть в положении `true`.
4. **Если виджета по-прежнему нет:** Переведите параметр `browser.newtabpage.activity-stream.nova.enabled` в положение `false`.
5. **Крайние меры:** Если ничего из вышеперечисленного не помогло, в сети существует несколько ритуалов для призыва виджета погоды Firefox. Возможно, придется прибегнуть к одному из них!
</details>

<details>
<summary><b>Скриншоты</b></summary>
Режим "Расширяющийся пробел", заголовок страницы включён, все виджеты включены:
<br><br>

![AllIn1](images/dynamic_spring.png)

Режим "Только виджеты", включены виджет погоды без иконки и виджет даты, у виджета даты подсказка с праздниками:
