...
(function() {
	if (window.uc_insertSymbols_initialized) return;
	window.uc_insertSymbols_initialized = true;

	const BUTTON_ID = "uc-insert-symbols-button";
	...
	function insertSymbol(sym) {
			...
			if (activeEl && activeEl.localName === "browser" && activeEl.isRemoteBrowser) {
				try {
					...
					// Это защищает Юникод от разрушения при передаче через data: URI
					let safeSym = "";
					for (let i = 0; i < sym.length; i++) {
						safeSym += "\\u" + ("0000" + sym.charCodeAt(i).toString(16)).slice(-4);
					}

					// Упаковываем полностью безопасный ASCII-код
	...
	CustomizableUI.createWidget({
		...
		onCreated: function(btn) {
			...
			// Обработка СКМ на кнопке панели
			...
			// Генерация сетки
			columns.forEach(colSymbols => {
				let menugroup = doc.createXULElement("menugroup");
				menugroup.setAttribute("orient", "vertical");
				menugroup.setAttribute("style", "margin: 2px 6px; min-width: 40px;");

				colSymbols.split(" ").forEach(sym => {
					if (!sym) return;

					let menuitem = doc.createXULElement("menuitem");
					menuitem.setAttribute("label", sym);
